...
im = Image.open('C:/Users/student/machineLearning/test.png')
...
cropImage.save('testcentercrop.jpg')
...
list = os.listdir('C:/Users/student/Downloads/강의자료/9. 머신러닝/GTSRB_Final_Training_Images/GTSRB/Final_Training/Images/00000')  # 현재 경로 폴더의 파일들을 불러옴
for item in list:
...
for item in list:
    filename, fileExtension = os.path.splitext(item)
    while(fileExtension == '.ppm'):
        im = Image.open(filename+fileExtension).convert('L')

        im = center_crop(im).resize((32,32))
...
a.tofile('foo.scv', sep=',', format='%10.5f')
# 또는
# np.savetxt('bar.scv', a, fmt='%10.5f', delimiter=',')